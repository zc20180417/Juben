...
| `~start <batch_id>` | 启动总入口：prepare → writer stage；writer 完成后停止，下一步进入 `check → aligner verify → verify-done → run` | `python _ops/controller.py start <batch_id>` |
| `~run <batch_id>` | 正式发布入口：lint gate → verify gate → promote → validate / review / next | `python _ops/controller.py run <batch_id>` |
| `~check <batch_id>` | 只跑 lint gate，并输出 verify 计划与 aligner 指令 | `python _ops/controller.py check <batch_id>` |
| `~finish <batch_id>` | Deprecated alias；兼容旧入口，等价于 `~run <batch_id>` | `python _ops/controller.py finish <batch_id>` |
...
- `~run` 是 Harness V2 的唯一正式发布入口；`~finish` 只保留兼容，不再作为推荐命令
